...
public class UpdatePromotionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String userAssign = admin.getUsername();

            String dateAssign = java.time.LocalDate.now().toString();

            PromotionDAO promotionDAO = new PromotionDAO();

            for (UserDTO userDTO : items.keySet()) {
                String idPromotion = UUID.randomUUID().toString();
                int rank = items.get(userDTO);
                String username = userDTO.getUsername();
                PromotionDTO promotion = new PromotionDTO(idPromotion, rank, username,dateAssign);
